  ...
  (it "creates scoped meters and collects synchronous and observable instruments"
    (let* ((provider
      ...
      (let* ((snapshots (metric-snapshot provider))
        ...
        (expect (metric-snapshot-temporality counter-snapshot)
                :to-equal :cumulative)
        (expect (metric-snapshot-timestamp counter-snapshot) :to-be-truthy)
        (expect (metric-snapshot-start-time counter-snapshot) :to-be-truthy)
        (expect (metric-sample-timestamp counter-sample)
                :to-equal (metric-snapshot-timestamp counter-snapshot))
        (expect (metric-sample-start-time counter-sample)
                :to-equal (metric-snapshot-start-time counter-snapshot))
        (expect (metric-snapshot-temporality observable-snapshot)
                :to-be-falsy)
        (expect (metric-snapshot-timestamp observable-snapshot)
                :to-be-truthy)
        (expect (metric-snapshot-start-time observable-snapshot)
                :to-be-falsy)
        (expect (metric-sample-timestamp observable-sample)
                :to-equal (metric-snapshot-timestamp observable-snapshot))
        (expect (metric-sample-start-time observable-sample)
                :to-be-falsy)
    ...
    (let* ((registry (make-metric-registry))
      ...
      (multiple-value-bind (snapshots collected-p)
        ...
        (expect (length (metric-reader-last-snapshots reader))
                :to-equal 1))
      (expect (force-flush-metric-reader reader) :to-be-truthy)
      (expect (shutdown-metric-reader reader) :to-be-truthy)
      (expect (metric-reader-shutdown-p reader) :to-be-truthy)
      (multiple-value-bind (snapshots collected-p)
          (collect-metric-reader reader)
        (expect snapshots :to-be-falsy)
